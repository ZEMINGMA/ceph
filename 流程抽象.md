...
| 类别 | 需求 | 说明 | 
|------|------|------|
| **必需** | **高速 CRC32 计算** | 在 `prepare_send_message()` 流程中，对 `front_crc` / `data_crc` 实现校验。 |
| | **消息头字段构建** | 自动填写 `ceph_msg_header` 中的 `data_len / crc / seq / tid` 等字段并写回缓冲区，确保 OSD 能正确解析。 |
| | **AES-GCM 加密** | 支持 Messenger v2 secure mode，对 front+data 进行流式加密 / 验签。 | 
| | **多级发送队列 & 优先级** | 保证与 AsyncConnection 优先级字段映射。 |
...
| | **大页 / HugeTLB** | 支持大页以便注册大块 buffer，提高 DMA 映射效率。 | 
