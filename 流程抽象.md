...
1. 将高级抽象参数转换为底层实现类型：
   Object → object_t
   IOContext → IOContextImpl
   WriteOp → OpImpl
2. 合并操作标志(flags = op->op.flags | ioc->extra_op_flags)
   设置修改时间(mtime)
3. 调用 objecter->mutate 提交实际写入：
   传递对象ID、位置、操作、快照上下文等
   设置修改时间、操作标志
   传递完成回调、对象版本、跟踪信息
...
1. 创建新的 Op 对象，封装所有变更参数：
   对象ID (oid)
   对象定位信息 (oloc)
   操作列表 (通过 std::move 转移所有权)
   合并操作标志 (flags | global_op_flags | CEPH_OSD_FLAG_WRITE)
   完成回调 (oncommit)
   对象版本指针 (objver)
   跟踪信息 (parent_trace)
2. 操作属性设置
...
op_submit中调用_op_submit_with_budget，该函数主要负责操作预算管理和超时控制,进一步调用_op_submit。_op_submit主要执行三个操作计算需要提交到哪个osd上（_calc_target(&op->target, nullptr)），根据osd号建立连接（_get_session(op->target.osd, &s, sul)），发送请求（_send_op(op)）。在_send_op中会构建MOSDOp对象（MOSDOp *m = _prepare_osd_op(op)，加参数和设置必要参数），发送请求（op->session->con->send_message(m)）
...
- **是否涉及数据：是。** 这一阶段对用户数据的处理包括：计算CRC，可能的加密，以及数据从用户内存到内核的复制和网络I/O。
...
简而言之，Ceph 客户端在写入时主要做了四件事：1）解析用户写入请求、拆分定位到对象（控制层）；2）必要时准备数据（如从父卷读取旧数据）；3）将请求提交给集群目标（协调通信，不碰数据）；4）对数据进行传输层处理并发送给OSD（对数据进行校验、加密并发送，重数据处理）。这些步骤中，决策和协调属于控制面，数据传输和处理属于数据面。